...
<x-slot name="header">
    <p class="w-1/2 mx-0.5 text-left font-bold text-base sm:text-lg">Edit Users</p>
    <p class="w-full m-0.5 text-right">
        <a href="{{route('admin.dashboard')}}" class="hover:text-cyan-500 hover:underline">Admin</a>
        /
        <a href="{{route('admin.users')}}" class="hover:text-cyan-500 hover:underline">User</a>
    </p>
</x-slot>
        ...
        <table class="w-full border-collapse border border-slate-400">
            ...
            <tbody>
                ...
                <tr class="text-sm sm:text-base">
                    ...
                    <td class="border border-slate-300 px-1">{{$user->name}} @if($agent->isDesktop())({{$user->email}})@endif</td>
                    <td class="border border-slate-300 px-1 text-center">{{$user->user_type}}</td>
                    <td class="text-center border border-slate-300 w-1/4">
                        <button wire:click="edit({{$user->id}})" class="py-2 px-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-400 focus:outline-none focus:border-blue-600 focus:shadow-outline-red active:bg-blue-500 transition ease-in-out duration-150"><i class="fa-regular fa-pen-to-square"></i></button>
                        <x-jet-delete-button id="{{$user->id}}" wire:click="delete({{$user->id}})" 
