...
<script>
    ...
    // var tvalue = $('#title').val();
        var tvalue = nameuser;
        var spvalue = tvalue.split('_-_');
        var user_id = spvalue[0];
        var user_name = spvalue[1];
        if(tvalue != ""){
            //Check maximum number of input fieldsconsole.log(add_div)
            var d_exist = $('#multiuser').find("option[value='" + user_id + "']").val() == user_id;
            console.log(d_exist);
            if(!d_exist){
                if(numb < maxField){ 
                    var add_div = "<option selected data-id='op-"+numb+"-ch' value='"+user_id+"'>"+user_name+"</option>";
                    var fieldHTML = "<div class='selection-choice op-"+numb+"-ch mx-1 my-1 max-w-sm w-auto' title='"+user_name+"'>"+
                                    "<span class='rounded border border-gray-300' style='background-color:#e5e7eb;'>"+
                                    '<a href="javascript:void(0);" data-id="op-'+numb+'-ch" onclick="delUser(`op-'+numb+'-ch`)" class="remove_button border-r border-gray-300 cursor-pointer text-md text-gray-400 hover:text-gray-700 px-1 my-1 hover:bg-gray-200 focus:bg-gray-100">x</a>'+
                                    '<span class="text-md my-1 mx-1">'+user_name+"</span></span></div>"; //New input field html 
                    console.log(add_div)
                    // console.log(fieldHTML)
                    numb++; //Increment field counter  
                    $('#multiuser').append(add_div);
                    $(wrapper).append(fieldHTML); //Add field html
                    $('#inloc').val("");
                    var multval = $('#multiuser').val();
                    window.livewire.emit('multiUser',multval)
                }else{
